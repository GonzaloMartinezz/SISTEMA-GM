...
import { Map, PackageSearch, PenTool, ArrowLeft, Settings } from 'lucide-react';
...
export default function LogisticsLayout() {
  ...
  const menuItems = [
    { name: 'Mapa de Rutas', path: '/logistics', icon: Map },
    { name: 'Entregas Pendientes', path: '/logistics/deliveries', icon: PackageSearch },
    { name: 'Mantenimientos', path: '/logistics/maintenance', icon: PenTool },
  ];
    ...
    <div className="flex h-screen bg-background overflow-hidden selection:bg-accent/30">
      <aside className="w-64 h-screen bg-surface border-r border-surfaceHighlight flex flex-col hidden md:flex sticky top-0">
        <div className="h-16 flex items-center px-4 border-b border-surfaceHighlight gap-3">
          ...
          <h1 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-accent to-pink-500 tracking-widest uppercase">
            Logística <span className="text-text">& Ops</span>
          </h1>
        </div>

        <nav className="flex-1 overflow-y-auto py-6 px-4 space-y-2">
          <p className="text-[10px] font-bold text-textMuted uppercase tracking-[0.2em] mb-4 pl-4">Operaciones</p>
          {menuItems.map((item) => (
            <NavLink
              key={item.name}
              to={item.path}
              ...
              className={({ isActive }) => cn(
                ...
                isActive 
                  ? "bg-accent/10 text-accent shadow-[inset_2px_0_0_#ff0055]" 
        ...
        <div className="p-4 border-t border-surfaceHighlight">
          <button className="flex items-center gap-3 px-4 py-2 w-full rounded-lg text-textMuted hover:bg-surfaceHighlight hover:text-text transition-colors">
            ...
            <span className="font-medium text-sm">Ajustes Logística</span>
          </button>
        </div>
      </aside>
